<nav class="flex flex-col fixed top-0 z-50 w-full">
    <div class="bg-custom-grey dark:bg-gray-900 w-full">
        <div class="w-full flex flex-row justify-between items-center text-gray-500 text-sm">
            <div class="flex items-center ms-12">
                {{-- <div class="flex"> --}}
                <a href="{{ route('dashboard') }}" class="space-x-3 rtl:space-x-reverse">
                    <div class="w-20 flex justify-center items-center">
                        <img src="{{ asset('image/logo.png') }}" alt="Logo CoETreemine" class="w-full h-full">
                    </div>
                </a>
                <div class="flex flex-col">
                    <h1 class="text-buyer-green font-semibold">PCB Prototype and Assembly the Easy Way</h1>
            ...
            <div class="flex items-center gap-4 me-12">
                ...
                <div class="flex gap-4">
                    <!-- <div class="flex items-center justify-end gap-2">
                        ...
                        <h1 id="user-menu-name" class="font-bold cursor-pointer">{{ Auth::user()->name }}</h1>
                        <script>
                            document.getElementById('user-menu-name').addEventListener('click', function() {
                    ...
                    <div>
                        <button type="button" class="flex text-sm rounded-full focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600" aria-expanded="false" data-dropdown-toggle="dropdown-user">
                            <span class="sr-only">Open user menu</span>
                            <img class="w-8 h-8 rounded-full" src="{{ asset('image/user.jpeg') }}" alt="user photo">
                            <h1 class="m-2 font-bold">Hi, {{ Auth::user()->name }}</h1>
                        </button>
                    </div>
                        ...
                        <ul class="py-2" aria-labelledby="user-menu-button">
                            ...
                            <li>
                                <form method="POST" action="{{ route('logout') }}" class="w-full">
                                    @csrf
                                    <button type="submit" class="text-start block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-600 dark:hover:text-white w-full" role="menuitem">Sign out</button>
                                </form>
                            </li>
                        </ul>
                    </div>
            ...
            <div class="flex gap-4">
                <div class="hover-effect ">
                    <a href="/login" class="font-bold flex items-center justify-end gap-1">
                        <i class="fas fa-user text-"></i>Sign In
                    </a>
                </div>
            ...
            <li class="relative {{ Route::is('buyer.assembly*') || Route::is('buyer.prototype*') ? 'border-b-2 border-white hover:text-black hover:border-black' : '' }}  hover-effect ">
                ...
                <button id="dropdownNavbarLink" data-dropdown-toggle="dropdownNavbar" class="flex items-center py-2 px-3 hover:text-black md:hover:bg-transparent md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 ml-1 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M6 6a.75.75 0 0 0-1.06 1.06l4.5 4.5a.75.75 0 0 0 1.06 0l4.5-4.5A.75.75 0 0 0 14 6H6z" clip-rule="evenodd" />
                    </svg>
                </button>
                <!-- Dropdown Content -->
                <div id="dropdownNavbar" class="absolute hidden font-normal bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600 mt-1">
                    <ul class="py-2 text-sm text-gray-700 dark:text-gray-400">
                        <li>
                            <a href="{{ route('buyer.prototype.index') }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Prototype</a>
                        </li>
                        <li>
                            <a href="{{ route('buyer.assembly.index') }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Assembly</a>
                        </li>
                    </ul>
                </div>
            </li>
