    ...
    <div class="flex flex-row mb-8 -mt-4 gap-4">
        ...
        <div class="w-3/4">
            <div class="w-full py-4 flex justify-between gap-4">
                ...
                <button class="w-1/2 button-green" onclick="calculate()"><i
                        class="fas fa-calculator me-2"></i>Calculate</button>
            </div>
                ...
                <form action="{{ route('buyer.prototype.store') }}" method="POST"
                    ...
                    <div class="x_out flex flex-row w-full">
                        ...
                        <div class="w-2/3 inline-flex flex-row gap-4">
                            ...
                            <div class="flex items-center">
                                <input type="radio" name="x_out" id="x_out-accept" value="accept" class="hidden peer"
                                    checked>
                    ...
                    <div class="panel_description flex flex-row w-full hidden">
                        ...
                        <div class="w-2/3 ">
                            <div class="panel_description">
                                <textarea name="panel_description" id="panel_description" rows="4" class="textarea-form"
                        ...
                        <div class="route">
                            <select id="route_process" name="route" class="select-form">
                                @foreach ($datas['dataRoute'] as $index => $data)
                                    <option value="{{ old('route', $data->id_route) }}"
                                        {{ $index == 0 ? 'selected' : '' }}>
                    ...
                    <div class="design flex flex-row w-full">
                        ...
                        <div class="w-2/3 inline-flex flex-row gap-4">
                            <div class="flex items-center">
                                <input type="radio" id="design-1" name="design" value="1" class="hidden peer"
                                    checked>
                    ...
                    <div class="size flex flex-row w-full items-center">
                        ...
                        <input type="number" name="length" placeholder="Length" value="{{ old('length') }}"
                            class="input-number">
                        <label for="length" class="label-input-number">X</label>
                        <input type="number" name="width" placeholder="Width" value="{{ old('width') }}"
                            class="input-number">
                    ...
                    <div class="layer flex flex-row w-full">
                        ...
                        <div class="w-2/3 flex flex-row gap-4">
                            @foreach ($datas['dataLayer'] as $index => $data)
                                <div class="flex items-center">
                                    <input type="radio" name="layer" id="layer-{{ $index }}"
                                        value="{{ old('layer', $data->id_layer) }}" {{ $index == 0 ? 'checked' : '' }}
                                        class="hidden peer">
                            ...
                            @foreach ($datas['dataMaterial'] as $index => $data)
                                <div class="flex items-center">
                                    <input type="radio" name="material"
                                        value="{{ old('material', $data->id_material) }}" class="hidden peer" checked
                                        {{ $index == 0 ? 'checked' : '' }}>
                    ...
                    <div class="thickness flex flex-row w-full">
                        ...
                        <div class="w-2/3 flex flex-row gap-4">
                            @foreach ($datas['dataThickness'] as $index => $data)
                                <div class="flex items-center">
                                    <input type="radio" name="thickness" id="thickness-{{ $index }}"
                                        value="{{ old('thickness', $data->id_thickness) }}"
                                        {{ $index == 0 ? 'checked' : '' }} class="hidden peer">
                ...
                {{-- @foreach ($datas['dataThicknessLayer2'] as $index => $data) --}}
                    ...
                    <div class="track flex flex-row w-full">
                        ...
                        <div class="w-2/3 flex flex-row gap-4">
                            @foreach ($datas['dataTrack'] as $index => $data)
                                <div class="flex items-center">
                                    <input type="radio" name="track" id="track-{{ $index }}"
                                        value="{{ old('track', $data->id_track) }}" {{ $index == 0 ? 'checked' : '' }}
                                        class="hidden peer">
                    ...
                    <div class="hole flex flex-row w-full">
                        ...
                        <div class="w-2/3 flex flex-row gap-4">
                            @foreach ($datas['dataHole'] as $index => $data)
                                <div class="flex items-center">
                                    <input type="radio" name="hole" id="hole-{{ $index }}"
                                        value="{{ old('hole', $data->id_hole) }}" {{ $index == 0 ? 'checked' : '' }}
                                        class="hidden peer">
                    ...
                    <div class="solder flex flex-row w-full">
                        ...
                        <div class="w-2/3 flex flex-row gap-4">
                            @foreach ($datas['dataSolder'] as $index => $data)
                                <div class="flex items-center">
                                    <input type="radio" name="solder" id="solder-{{ $index }}"
                                        value="{{ old('solder', $data->id_solder) }}" {{ $index == 0 ? 'checked' : '' }}
                                        class="hidden peer">
                    ...
                    <div class="silk flex flex-row w-full">
                        ...
                        <div class="w-2/3 flex flex-row gap-4">
                            @foreach ($datas['dataSilk'] as $index => $data)
                                <div class="flex items-center">
                                    <input type="radio" name="silk" id="silk-{{ $index }}"
                                        value="{{ old('silk', $data->id_silk) }}" {{ $index == 0 ? 'checked' : '' }}
                                        class="hidden peer">
                    ...
                    <div class="surface flex flex-row w-full">
                        ...
                        <div class="w-2/3 flex flex-row gap-4">
                            @foreach ($datas['dataSurface'] as $index => $data)
                                <div class="flex items-center">
                                    <input type="radio" name="surface" id="surface-{{ $index }}"
                                        value="{{ old('surface', $data->id_surface) }}"
                                        {{ $index == 0 ? 'checked' : '' }} class="hidden peer">
                    ...
                    <div class="via_process flex flex-row w-full">
                        ...
                        <div class="w-2/3 flex flex-row gap-4">
                            @foreach ($datas['dataViaProcess'] as $index => $data)
                                <div class="flex items-center">
                                    <input type="radio" name="via" id="via-{{ $index }}"
                                        value="{{ old('via', $data->id_via) }}" {{ $index == 0 ? 'checked' : '' }}
                                        class="hidden peer">
                    ...
                    <div class="finished_cooper flex flex-row w-full">
                        ...
                        <div class="w-2/3 flex flex-row gap-4">
                            @foreach ($datas['dataFinishedCooper'] as $index => $data)
                                <div class="flex items-center">
                                    <input type="radio" name="finished" id="finished-{{ $index }}"
                                        value="{{ old('finished', $data->id_finished_cooper) }}"
                                        {{ $index == 0 ? 'checked' : '' }} class="hidden peer">
                    ...
                    <div class="inner_cooper flex flex-row w-full">
                        ...
                        <div class="w-2/3 flex flex-row gap-4">
                            @foreach ($datas['dataInnerCooper'] as $index => $data)
                                <div class="flex items-center">
                                    <input type="radio" name="inner" id="inner-{{ $index }}"
                                        value="{{ old('inner', $data->id_inner) }}" {{ $index == 0 ? 'checked' : '' }}
                                        class="hidden peer">
                    ...
                    <div class="flex justify-end mt-4 gap-4">
                        ...
                        <button type="submit" class="w-1/6 button-green"><i class="fas fa-shopping-cart me-2"></i>Add to
                            Cart</button>
                        <button class="w-1/6 button-green" onclick="calculate()"><i
                                class="fas fa-calculator me-2"></i>Calculate</button>
                    </div>
                </form>
            </div>
        ...
        <div class="w-1/4 py-4 px-8 mt-4 bg-white rounded-lg shadow-lg h-1/6">
            ...
            <div id="calculate-section">
                ...
                <div class="flex flex-row justify-center text-sm whitespace-nowrap truncate">
                    <button class="button-green w-32 " onclick="calculate()"><i
                            class="fas fa-calculator me-2"></i>Calculate</button>
                </div>
            </div>
            <!-- Add to Cart Section -->
            <div id="add-to-cart-section" class="hidden">
                ...
                <div class="result flex flex-row justify-center items-center text-sm whitespace-nowrap truncate">
                    <button type="submit" class="button-green" onclick="calculate()"><i
